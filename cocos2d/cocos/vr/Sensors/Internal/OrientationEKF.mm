...
GLKMatrix4 OrientationEKF::getPredictedGLMatrix(double secondsAfterLastGyroEvent)
{
    ...
    Vector3d *pmu = new Vector3d(this->lastGyro.v[0] * -dT, this->lastGyro.v[1] * -dT, this->lastGyro.v[2] * -dT);
...
void OrientationEKF::processGyro(GLKVector3 gyro, double sensorTimeStamp)
{
    if (this->sensorTimeStampGyro != 0.0) {
        ...
        Vector3d *motionVector = new Vector3d(gyro.v[0] * -dT, gyro.v[1] * -dT, gyro.v[2] * -dT);
...
{
    Vector3d *motionVector = new Vector3d(acc.v[0], acc.v[1], acc.v[2]);
